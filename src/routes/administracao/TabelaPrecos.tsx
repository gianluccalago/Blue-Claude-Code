...
export function TabelaPrecos() {
  ...
  const porChave = new Map((data ?? []).map((p) => [chavePreco(p.tipo_suite, p.grau, p.ocupacao), p]));
      ...
      <Card>
        ...
        <CardContent className="space-y-5">
          <p className="text-sm text-muted-foreground">
            ...
            combinações). Esses valores sugerem a mensalidade dos hóspedes em "Mensalidades",
              ...
              {ocupacoesValidas(tipo).map((ocupacao) => (
                <div key={ocupacao} className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                    {GRAUS.map((grau) => {
                      const item = porChave.get(chavePreco(tipo, grau, ocupacao));
                      if (!item) return null;
